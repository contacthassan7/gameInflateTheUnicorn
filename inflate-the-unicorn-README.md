...
Passing a parameter from the html tag via onclick in js function and than using it to identify the elements. I could have added an id on the elements but decided to do it this way as i needed to number the unicorns at the end. I didnt remember how to alert so the link was really helpfull. 

- Did you encounter issues completing any of the steps of this exercise? How did you resolve this?
the most challenging part was to split the returns of the js function, i read about a things called 'this', as far as i could understand it can help with returning the function results to the element calling it and not to everyone. Like i got all the elements using the same class so the function change was effecting all three unicorns in the UI. adding a number parameter in the function call helped me identify the element number in my array.

- If you were to replace the unicorn with another mystical animal of your choice, what would it be?  
jumping monkey may be
